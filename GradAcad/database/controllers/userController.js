...
export const getCountUsersRole = async (req, res) => {
  try {
    ...
    // Aggregate query to count users based on their role
    ...
    // Convert the result into a more user-friendly format
    const formattedCounts = {};
    roleCounts.forEach(({ _id, count }) => {
      formattedCounts[_id] = count;
    });

    res.status(200).json({ success: true, roleCounts: formattedCounts });
  } catch (error) {
